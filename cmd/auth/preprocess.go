...
func preprocess(ev events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if err != nil {
		...
		return newResponse("Wrong authorization header", http.StatusUnauthorized), errors.New("claims")
	}
...
func generateState(ev events.APIGatewayProxyRequest) string {
	...
	log.Print("Making envelope for token....")
	// wrap token for use as state value
	ciphertext := encryptState(token)
	log.Printf("Serialized to - %s", ciphertext)
	return ciphertext
}
