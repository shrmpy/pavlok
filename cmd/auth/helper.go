...
// unmarshal the claims struct from the token
func (s *service) claims(token string) (*jwtClaims, error) {

	parsedToken, err := s.parser.ParseWithClaims(token, &jwtClaims{}, s.getKey)

	if err != nil {
		log.Print("Could not parse authorization header")
		log.Print(err)
		return &jwtClaims{}, err
	}

	if cl, ok := parsedToken.Claims.(*jwtClaims); ok && parsedToken.Valid {
		return cl, nil
	}

	log.Print("Cast JWT claims failed")
	return &jwtClaims{}, errors.New("Cast JWT claims failed")
